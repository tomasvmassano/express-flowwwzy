...
export const LIBRARY: ComponentManifest[] = [
  ...
  // ─────────────────────────────────────────────────────────────────────
  {
    ...
    slots: [
      ...
      {
        key: "benefits",
        label: "2-6 benefits (icon, title, body)",
        type: "list",
        required: true,
        hint: 'Each item: { icon: "clock"|"tag"|"spark"|"shield"|"zap"|"check" (or omit if none fits), title: string, body: string }. The icon string MUST be exactly one of the listed values — no other names accepted.',
      },
    ],
